...
4. **Persistent storage**: DynamoDB holds every decision and every agent's structured output. An S3 bucket exists for evidence, provisioned and IAM-permissioned, but not yet the active storage backend (see [Limitations](../README.md#29-limitations)).
