...
- If a run produces zero recommended experiments (the Experiment Planner is explicitly allowed to decline rather than fabricate one — see [`README.md` Limitations](../README.md#26-limitations)), re-run the analysis rather than improvising a fake experiment on screen. The demo data in `docs/DEMO_DATA.md` is chosen specifically because it reliably produces a rich threshold + experiment story.
