  ...
  return (
    <div className="flex items-center justify-between gap-4 border-b border-hairline px-4 py-3">
      ...
      <div className="flex shrink-0 items-center gap-1.5">
        <span className="h-5 w-14 rounded-sm bg-white/5" />
        <span className="h-5 w-16 rounded-sm bg-gradient-to-b from-ink to-ink/80" />
      </div>
    </div>
  );
}
...
function StatTile({ label, value, tone }: { label: string; value: string; tone: string }) {
  return (
    <div className="rounded-md border border-hairline bg-white/[0.02] p-2.5">
    ...
    <div className="flex-1 space-y-3 p-4">
      ...
      <div className="rounded-md border border-hairline bg-white/[0.02] p-3">
        <div className="mb-3 flex items-center justify-between">
          <p className="text-[0.625rem] font-semibold text-ink">Regret exposure by decision</p>
      ...
      <div className="grid grid-cols-2 gap-2">
        {[
          ...
          <div key={label} className="space-y-1">
            ...
            <div className="rounded-md border border-hairline bg-canvas px-2.5 py-1.5 text-[0.5625rem] text-ink">
              {value}
            </div>
          </div>
        ))}
  ...
  return (
    <div className="flex-1 space-y-2 p-4">
      {AGENTS.map(([name, detail, state]) => (
        <div
          key={name}
          className="flex items-center gap-2.5 rounded-md border border-hairline bg-white/[0.02] px-2.5 py-2"
        >
          <span
            className={cn(
          ...
          <p className="shrink-0 text-[0.5625rem] text-ink-secondary">{detail}</p>
        </div>
      ))}
...
function ReportBody() {
  return (
    <div className="flex-1 space-y-3 p-4">
      <div className="rounded-md border border-warning-line bg-panel-warning p-3">
      ...
      <div className="rounded-md border border-hairline bg-white/[0.02] p-3">
        <div className="flex items-center justify-between">
          <p className="text-[0.625rem] font-semibold text-ink">Repeat customer rate</p>
        ...
        <div className="mt-2 flex items-center justify-between text-[0.5625rem]">
          <span className="text-info-ink">evidence 18–21%</span>
          <span className="text-danger-ink">breaks below 24%</span>
        </div>
      </div>

      <div className="rounded-md border border-accent-line bg-panel-accent p-3">
...
export function MockScreen({ variant = 'dashboard' }: { variant?: MockVariant }) {
  ...
  return (
    <div className="flex h-full min-h-0 overflow-hidden rounded-lg border border-canvas bg-surface">
      <Sidebar />
...
export function MockFrame({ variant = 'dashboard', fade = false, className }: MockFrameProps) {
  return (
    <div aria-hidden className={cn('relative', className)}>
      <div className="h-full rounded-2xl bg-white/10 p-2 pb-0">
