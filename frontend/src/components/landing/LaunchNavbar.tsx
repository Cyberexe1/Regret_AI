...
export function LaunchNavbar() {
  ...
  useOutsidePointerDown(open, headerRef, close);

  /**
   * The sheet is `lg:hidden`, so at desktop widths it vanishes visually while
   * its state says otherwise. Reset it, or the menu reappears unprompted the
  ...
  return (
    <header ref={headerRef} className="sticky top-0 z-40 glass">
          ...
          >
            <div className="mb-3 space-y-1 rounded-xl border border-white/10 bg-surface p-3 shadow-soft">
              ...
              <div className="flex flex-col gap-2 pt-2 sm:hidden">
                <Link to={ROUTES.newDecision} onClick={close} className={cn(BTN_LIGHT, BTN_SHEET)}>
                  Get started
                </Link>
                <Link to={ROUTES.dashboard} onClick={close} className={cn(BTN_GLASS, BTN_SHEET)}>
                  Sign in
                </Link>
              </div>
            </div>
