...
export function CalibrationInsightsCard({ rows, isLoading = false }: CalibrationInsightsCardProps) {
  ...
  return (
    <Card padding="md" className="flex h-full min-w-0 flex-col overflow-hidden">
      ...
      {isLoading ? (
        <div className="mt-3 space-y-3" role="status" aria-label="Checking your calibration history">
        ...
        <ol className="mt-3 min-w-0 space-y-3">
          {topRows.map((row) => (
            <li key={row.calibrationId} className="min-w-0 border-t border-hairline pt-3 first:border-t-0 first:pt-0">
              <div className="flex min-w-0 flex-wrap items-start justify-between gap-2">
                <p className="min-w-0 flex-1 break-words text-small text-ink">{row.variable}</p>
                <Badge tone={row.biasTone} size="sm" dot>
                  {row.biasLabel}
                </Badge>
              </div>
              <p className="mt-1.5 break-words text-micro text-ink-muted">{row.explanation}</p>
            </li>
          ))}
        </ol>
      )}
