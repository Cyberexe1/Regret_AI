...
/**
 * Deletion is deliberately inert - this action is not wired to any
 * backend delete endpoint. The modal states plainly that nothing is
...
export function DangerZone() {
  ...
  const decisionCount = dashboard.data?.decisions.length ?? 0;
  const experimentCount = dashboard.data
    ? [...dashboard.data.experimentsByDecisionId.values()].reduce((sum, list) => sum + list.length, 0)
    : 0;
    ...
    <div className="px-5 py-5 md:px-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-1 max-w-md text-small text-ink-secondary">
            Would remove every decision, analysis and experiment in this workspace. There is no
            delete-everything endpoint wired up yet, so this action is currently inert.
          </p>
        </div>
      ...
      {acknowledged ? (
        <p className="mt-4 flex items-start gap-2 rounded-lg border border-hairline bg-surface-inset px-4 py-3 text-small text-ink-secondary">
          ...
          Nothing was deleted. This action is not connected to a delete endpoint yet.
        </p>
      ) : null}
      ...
      >
        <ul className="space-y-2">
          <li className="flex items-baseline justify-between gap-4 text-small">
            ...
            <span className="numeric font-medium text-ink">{decisionCount}</span>
          </li>
          <li className="flex items-baseline justify-between gap-4 text-small">
            <span className="text-ink-secondary">Experiments</span>
            <span className="numeric font-medium text-ink">{experimentCount}</span>
          </li>
        </ul>

        <p className="mt-5 rounded-lg border border-info-line bg-panel-info px-4 py-3 text-small text-ink-secondary">
          Nothing will actually be deleted. There is no delete-everything endpoint wired up yet,
          so confirming only demonstrates the flow.
        </p>
      </Modal>
    </div>
  );
